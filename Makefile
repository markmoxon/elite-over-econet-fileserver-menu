...
all:
	...
	$(BEEBASM) -i 1-source-files/main-sources/elite-disc.asm -do 5-compiled-game-discs/elite-over-econet-server-menu.ssd -boot MENU -title "E L I T E"
ifneq ($(verify), no)
...
b2:
	...
	curl -H "Content-Type:application/binary" --upload-file "5-compiled-game-discs/elite-over-econet-server-menu.ssd" "http://localhost:48075/run/b2?name=elite-over-econet-server-menu.ssd"
